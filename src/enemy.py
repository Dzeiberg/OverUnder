...
class Enemy(pygame.sprite.Sprite):
	def __init__(self,x,y):
		#initalize the animations for the enemy
		...
		self.leftIDX = 0
		self.rightIDX = 0
		#just kept the old names for the images
	...
	def update(self, platform_list, players):
		...
		#checks if the enemy collides with anything
		#nearly identical to the player collision logic
		...
		for block in collision_list:
			if isinstance(block,Level.Spike):
				...
				#if moving left, place the enemy to the right of the platform
				if self.speedX > 0:
					self.rect.right = block.rect.left
				#if moving right, place the enemy to the left of the platform
				elif self.speedX < 0:
					self.rect.left = block.rect.right
				self.crouch()
			#if it's Dirt
		...
		for block in collision_list:        
			...
			if isinstance(block, Level.Platform):
				#If the enemy is under a wall, stop the enemy's and the wall's movement
		...
		#returns True for if the enemy touched the respective player
		return [dead1,dead2]   		
	...
	def crouch(self):
		#create new sprite for crouching enemy
	...
	#animates the enemy, very similar to the player class
	def animate(self):
		#every three frames
		if self.ticker%3 == 0:
			self.image.set_colorkey(pygame.Color("white"))
	...
	#figures out how the enemy should move based on the locations of the other two players
	def getSpeed(self,playerOne,playerTwo):
		...
		#if they're the same distance away, stop moving
		else:
			self.speedX = 0